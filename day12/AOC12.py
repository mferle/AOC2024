...
session.file.put("AOC12_example.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
session.file.put("AOC12_input.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
...
@sproc(is_permanent=True, 
       ...
       imports=['AOC12_part1_part2.py'])
def AOC12(session: Session, fileurl: str) -> str:
    from snowflake.snowpark.files import SnowflakeFile
    from AOC12_part1_part2 import Part1Part2
...
lines_df = session.sql("""call AOC12(build_scoped_file_url(@aoc_files_stage, 'AOC12_input.txt'))""")
lines_df.show()

# to test locally, comment the previous two lines and the sproc decorator
#print(AOC12(session, 'AOC12_input.txt'))
