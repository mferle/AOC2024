...
session.file.put("AOC10_example.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
session.file.put("AOC10_input.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
...
@sproc(is_permanent=True, 
       ...
       imports=['AOC10_part1_part2.py'])
def AOC10(session: Session, fileurl: str) -> str:
    from snowflake.snowpark.files import SnowflakeFile
    from AOC10_part1_part2 import Part1Part2
...
lines_df = session.sql("""call AOC10(build_scoped_file_url(@aoc_files_stage, 'AOC10_example.txt'))""")
lines_df.show()

# to test locally, comment the previous two lines and the sproc decorator
#print(AOC10(session, 'AOC10_example.txt'))
