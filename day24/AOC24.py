...
session.file.put("AOC24_example.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
session.file.put("AOC24_input.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
...
@sproc(is_permanent=True, 
       ...
       imports=['AOC24_part1_part2.py'])
def AOC24(session: Session, fileurl: str) -> str:
    from snowflake.snowpark.files import SnowflakeFile
    from AOC24_part1_part2 import Part1Part2
...
lines_df = session.sql("""call AOC24(build_scoped_file_url(@aoc_files_stage, 'AOC24_input.txt'))""")
lines_df.show()

# to test locally, comment the previous two lines and the sproc decorator
#print(AOC24(session, 'AOC24_input.txt'))
