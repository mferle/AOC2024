...
session.file.put("AOC21_example.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
session.file.put("AOC21_input.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
...
@sproc(is_permanent=True, 
       ...
       imports=['AOC21_part1_part2.py'])
def AOC21(session: Session, fileurl: str) -> str:
    from snowflake.snowpark.files import SnowflakeFile
    from AOC21_part1_part2 import Part1Part2
...
lines_df = session.sql("""call AOC21(build_scoped_file_url(@aoc_files_stage, 'AOC21_input.txt'))""")
lines_df.show()

# to test locally, comment the previous two lines and the sproc decorator
#print(AOC21(session, 'AOC21_input.txt'))
