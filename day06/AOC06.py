...
session.file.put("AOC06_example.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
session.file.put("AOC06_input.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
...
@sproc(is_permanent=True, 
       ...
       imports=['AOC06_part1_part2.py'])
def AOC06(session: Session, fileurl: str) -> str:
    from snowflake.snowpark.files import SnowflakeFile
    from AOC06_part1_part2 import Part1Part2
...
lines_df = session.sql("""call AOC06(build_scoped_file_url(@aoc_files_stage, 'AOC06_example.txt'))""")
lines_df.show()

# to test locally, comment the previous two lines and the sproc decorator
#print(AOC06(session, 'AOC06_input.txt'))
