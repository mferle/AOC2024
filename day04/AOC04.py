...
#session.file.put("AOC04_example.txt", "@aoc_files_stage", auto_compress=False)
#session.file.put("AOC04_input.txt", "@aoc_files_stage", auto_compress=False)
...
@sproc(is_permanent=True, 
       ...
       imports=['AOC04_part1_part2.py'])
def AOC04(session: Session, fileurl: str) -> str:
    from snowflake.snowpark.files import SnowflakeFile
    from AOC04_part1_part2 import Part1Part2
...
lines_df = session.sql("""call AOC04(build_scoped_file_url(@aoc_files_stage, 'AOC04_input.txt'))""")
lines_df.show()

# to test locally, comment the previous two lines and the sproc decorator
#print(AOC04(session, 'AOC04_example.txt'))
