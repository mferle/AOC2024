...
session.file.put("AOC23_example.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
session.file.put("AOC23_input.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
...
@sproc(is_permanent=True, 
       ...
       imports=['AOC23_part1_part2.py'])
def AOC23(session: Session, fileurl: str) -> str:
    from snowflake.snowpark.files import SnowflakeFile
    from AOC23_part1_part2 import Part1Part2
...
lines_df = session.sql("""call AOC23(build_scoped_file_url(@aoc_files_stage, 'AOC23_input.txt'))""")
lines_df.show()

# to test locally, comment the previous two lines and the sproc decorator
#print(AOC23(session, 'AOC23_input.txt'))
