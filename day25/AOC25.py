...
session.file.put("AOC25_example.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
session.file.put("AOC25_input.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
...
@sproc(is_permanent=True, 
       ...
       imports=['AOC25_part1_part2.py'])
def AOC25(session: Session, fileurl: str) -> str:
    from snowflake.snowpark.files import SnowflakeFile
    from AOC25_part1_part2 import Part1Part2
    ...
    # there is no Part 2 in the last puzzle
    #part2_answer = _Part1Part2.part2()

    return_msg = f"Part 1 answer = {part1_answer}"
    return return_msg

lines_df = session.sql("""call AOC25(build_scoped_file_url(@aoc_files_stage, 'AOC25_input.txt'))""")
lines_df.show()

# to test locally, comment the previous two lines and the sproc decorator
#print(AOC25(session, 'AOC25_input.txt'))
