...
#session.file.put("AOC11_example.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
#session.file.put("AOC11_input.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
...
@sproc(is_permanent=True, 
       ...
       imports=['AOC11_part1_part2.py'])
def AOC11(session: Session, fileurl: str) -> str:
    from snowflake.snowpark.files import SnowflakeFile
    from AOC11_part1_part2 import Part1Part2
...
lines_df = session.sql("""call AOC11(build_scoped_file_url(@aoc_files_stage, 'AOC11_input.txt'))""")
lines_df.show()

# to test locally, comment the previous two lines and the sproc decorator
#print(AOC11(session, 'AOC11_input.txt'))
