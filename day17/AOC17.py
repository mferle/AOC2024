...
session.file.put("AOC17_example.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
session.file.put("AOC17_input.txt", "@aoc_files_stage", auto_compress=False, overwrite=True)
...
@sproc(is_permanent=True, 
       ...
       imports=['AOC17_part1_part2.py'])
def AOC17(session: Session, fileurl: str) -> str:
    from snowflake.snowpark.files import SnowflakeFile
    from AOC17_part1_part2 import Part1Part2
...
lines_df = session.sql("""call AOC17(build_scoped_file_url(@aoc_files_stage, 'AOC17_input.txt'))""")
lines_df.show()

# to test locally, comment the previous two lines and the sproc decorator
#print(AOC17(session, 'AOC17_input.txt'))
